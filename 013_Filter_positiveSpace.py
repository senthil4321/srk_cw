...
for value in wave:
    ...
    medianFilterValue = medianFilter.update(value+1)
    medianFilterValues.append(medianFilterValue)  

    smoothingValue = smoothing.update(value+1)
    smoothingValues.append(smoothingValue)  


import holoviews as hv
hv.extension('bokeh')
curve = hv.Curve(values).opts(fontscale=2, width=2000, height=1000, title='Analysis of Power Trace')* hv.Curve(medianFilterValues)* hv.Curve(smoothingValues)
